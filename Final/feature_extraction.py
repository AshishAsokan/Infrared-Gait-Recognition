...
def calc_wavelet_component(haar_coeff, noof_frames):
    ...
    for index in range(len(haar_coeff)):
        ...
        coefficient_4 = [np.sum(element[3], axis=None) for element in haar_coeff[index]]
        ...
        # Calculating mean and standard deviation for 4th list of coefficients
        mean_4 = np.sum(coefficient_4, axis=None) / noof_frames[index]
        stand_4 = calc_stand_deviation(coefficient_4, noof_frames[index], mean_4)
        ...
        stand_value_4 += stand_4

    divide = len(haar_coeff)
    if len(haar_coeff) == 0:
        divide = 0.5
    mean_value_1 /= divide
    mean_value_2 /= divide
    ...
    stand_value_1 /= divide
    stand_value_2 /= divide
    stand_value_3 /= divide
    stand_value_4 /= divide

    return [mean_value_1, mean_value_2, mean_value_3, mean_value_4, stand_value_1, stand_value_2,
            stand_value_3, stand_value_4]
